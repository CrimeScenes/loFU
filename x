...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode[getgenv().KeyTrigger.TriggerBot:upper()] then
        isToggled = not isToggled  -- Toggle the TriggerBot on key press
        if isToggled then
            TriggerBotAction()  -- Perform action immediately on toggle
        end
    end
end)
...
RunService.RenderStepped:Connect(function()
    if isToggled then
        TriggerBotAction()  -- Continuously trigger while toggled
    end
end)
